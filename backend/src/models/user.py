...
class User(Base):
    ...
    # cascade="all, delete-orphan" + passive_deletes=True lets the ORM rely on
    # the `ON DELETE CASCADE` FK at the DB layer, avoiding SQLAlchemy attempting
    # to blank the PK of UserPreferences (its PK is also a FK to users.id).
